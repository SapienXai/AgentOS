...
test("the exact OpenClaw 2026.9.1 package and protocol remain authoritative", async () => {
  ...
  assert.equal(packageJson.dependencies?.["@openclaw/gateway-client"], "2026.9.1");
  assert.equal(packageJson.dependencies?.["@openclaw/gateway-protocol"], "2026.9.1");
