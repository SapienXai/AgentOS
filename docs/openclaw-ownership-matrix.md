...
| Unified native plugin catalog/operator projection | C | OpenClaw `plugins.catalog.*` records and native installed/enabled/local state; AgentOS owns only the typed read-only projection and recovery explanation. | [C94 contract diff](evidence/openclaw-2026.9.3-to-2026.9.4-contract-diff.json); [catalog service](../lib/openclaw/application/catalog-service.ts); [native payloads](../lib/openclaw/client/native-ws-gateway-payloads.ts); [catalog tests](../tests/openclaw-plugin-catalog.test.ts) | native + degraded | Keep browse/categories/detail Gateway-first with `operator.read`, preserve unsupported/denied/unknown states, and never add a parallel plugin database or install authority. |
