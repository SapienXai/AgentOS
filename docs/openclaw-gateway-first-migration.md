...
The OpenClaw CLI remains the bounded fallback implementation. AgentOS now consumes the public
`@openclaw/gateway-protocol@2026.8.2` package for protocol constants and capability names. The
AgentOS WebSocket client remains custom so it can preserve application-level fallback policy,
...
- Agent create/update/delete are Gateway-first when the Gateway advertises the lifecycle methods. Native `agents.create` receives only the 2026.8.2 fields it accepts (`name`, `workspace`, `model`, `emoji`, and `avatar`); AgentOS keeps its product-owned id/path, identity, policy skill, bootstrap, and workspace-manifest side effects outside that native payload.
