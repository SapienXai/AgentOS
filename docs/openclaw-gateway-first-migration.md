...
> Historical migration record. The active baseline is OpenClaw `2026.9.1`; see
> [`openclaw-2026.9.1-compatibility-audit.md`](./openclaw-2026.9.1-compatibility-audit.md)
...
`@openclaw/gateway-protocol@2026.9.1` package for protocol constants and capability names, and
the official `@openclaw/gateway-client@2026.9.1` owns the production transport. AgentOS retains
...
[`openclaw-2026.9.1-compatibility-audit.md`](./openclaw-2026.9.1-compatibility-audit.md). The
...
- Agent create/update/delete are Gateway-first when the Gateway advertises the lifecycle methods. Native `agents.create` receives only the 2026.9.1 fields it accepts (`name`, `workspace`, `model`, `emoji`, and `avatar`); AgentOS keeps its product-owned id/path, identity, policy skill, bootstrap, and workspace-manifest side effects outside that native payload.
