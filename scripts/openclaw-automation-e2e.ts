...
const OUTPUT_PATH = process.env.OPENCLAW_AUTOMATION_E2E_OUTPUT?.trim() || path.resolve("docs/evidence/openclaw-2026.8.2-automation-cron-alignment.json");
const REQUEST_TIMEOUT_MS = 10_000;

async function main() {
  if (!PACKAGE_INPUT) throw new Error("Set OPENCLAW_AUTOMATION_E2E_PACKAGE to an exact OpenClaw 2026.8.2 package root.");
  ...
  const evidence = {
    ...
    gate: "OPENCLAW 8.2 AUTOMATION/CRON ALIGNMENT GATE: FAIL",
    success: false
  };
  ...
  try {
    ...
    evidence.gate = success && evidence.cleanup.disposableRootRemoved && evidence.cleanup.gatewayProcessStopped
      ? "OPENCLAW 8.2 AUTOMATION/CRON ALIGNMENT GATE: PASS"
      : "OPENCLAW 8.2 AUTOMATION/CRON ALIGNMENT GATE: FAIL";
  ...
  console.log("OPENCLAW 8.2 AUTOMATION/CRON ALIGNMENT GATE: PASS");
