...
type ResolvedDependencies = Required<Pick<WorkspaceIntelligenceStatusDependencies, "creationRootPath" | "provisioningRootPath" | "now" | "readBinding" | "findProvisioningRun" | "findCreationRun" | "readPack" | "readComposition">> & {
...
export async function getWorkspaceIntelligenceStatus(
  ...
  const candidateRunId = input.candidateCreationRunId?.trim() || null;
  if (!candidateRunId) {
  ...
  const candidateLocator = await resolved.findCreationRun(resolved.creationRootPath, input.actorId, candidateRunId);
  ...
  if (!candidateLocator || !candidateResult || candidateLocator.run.snapshot.state !== "review-ready") {
    return statusWithBinding(workspaceId, checkedAt, binding, "The candidate workspace analysis is not ready for comparison.", "unknown", live, null);
  }
...
function emptyStatus(workspaceId: string, checkedAt: string, reason: string): WorkspaceIntelligenceStatus {
  return { workspaceId, status: "unknown", reason, checkedAt, binding: null, live: null, candidate: null, drift: null };
}
