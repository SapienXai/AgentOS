...
function ModelRow({ model, selected, onSelect, defaultModel, fallbackPosition }: { model: ModelManagementSnapshot["models"][number]; selected: boolean; onSelect: () => void; defaultModel: string | null; fallbackPosition: number }) {
  ...
  const unknown = model.available === null && !unavailable;
  const role = defaultModel?.toLowerCase() === model.id.toLowerCase() ? "Default" : fallbackPosition > 0 ? `Fallback ${fallbackPosition}` : unavailable ? "Unavailable" : unknown ? "Status unknown" : "Available";
...
function ProviderCard({ provider, models, canManageSecrets, onDisconnect, onDisconnectProfile }: { provider: ModelManagementProvider; models: Array<{ id: string; name: string; available: boolean | null }>; canManageSecrets: boolean; onDisconnect?: () => void; onDisconnectProfile: (profileId: string) => void }) {
...
function ModelInspector({ model, defaultModel, fallbackPosition, onClose }: { model: ModelManagementSnapshot["models"][number]; defaultModel: string | null; fallbackPosition: number; onClose: () => void }) {
  const statusUnknown = model.available === null && model.role !== "unavailable";
  const statusLabel = model.available === false || model.role === "unavailable" ? "Unavailable" : statusUnknown ? "Status unknown" : "Available";
