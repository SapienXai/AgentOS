...
export function HumanControlInbox({ surfaceTheme }: { surfaceTheme: SurfaceTheme }) {
  ...
  useEffect(() => {
    ...
    void loadInbox();
  }, [error, loading, open, payload]);

  async function loadInbox() {
    setLoading(true);
    setError(null);
    try {
      const response = await fetch("/api/human-control", { cache: "no-store" });
  ...
  return (
    <section className={cn("cockpit-panel rounded-xl border p-3", surfaceTheme === "dark" ? "border-border/80" : "border-border")}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-2.5">
          <div className="mt-0.5 rounded-lg border border-[hsl(var(--status-warning)/0.25)] bg-[hsl(var(--status-warning)/0.10)] p-2 text-[hsl(var(--status-warning-foreground))]">
            <Inbox className="h-4 w-4" aria-hidden="true" />
          </div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-foreground">Needs your attention</p>
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              {summary ? `${summary.totalPending} pending item${summary.totalPending === 1 ? "" : "s"}.` : "Review approvals, questions, and active blockers."}
            </p>
          </div>
        </div>
        <Button size="sm" variant="secondary" className="h-8 shrink-0 rounded-lg px-2.5 text-xs" onClick={() => setOpen(true)}>
          {summary ? "View all" : "Open queue"}
          <ChevronRight className="ml-1 h-3.5 w-3.5" aria-hidden="true" />
        </Button>
      </div>
      {summary ? <InboxSummary summary={summary} /> : null}

      <Dialog open={open} onOpenChange={(nextOpen) => { setOpen(nextOpen); if (nextOpen) { setPayload(null); setError(null); setAnswers({}); } }}>
        <DialogContent className="flex max-h-[min(88dvh,760px)] max-w-2xl flex-col gap-3 overflow-hidden rounded-[18px] p-4 sm:p-5">
          <DialogHeader className="pr-8">
            <DialogTitle className="text-lg">Needs your attention</DialogTitle>
            <DialogDescription className="text-xs leading-5">
              Current OpenClaw items that need an operator decision or action.
            </DialogDescription>
          </DialogHeader>
          ...
          {loading ? (
            ...
            <div className="rounded-lg border border-[hsl(var(--status-danger)/0.30)] bg-[hsl(var(--status-danger)/0.08)] p-3 text-sm text-[hsl(var(--status-danger-foreground))]" role="alert">
              {error}
              <Button variant="secondary" size="sm" className="mt-3 h-8 rounded-lg text-xs" onClick={() => { setPayload(null); void loadInbox(); }}>
                Try again
              </Button>
            </div>
            ...
            <div className="min-h-0 space-y-2 overflow-y-auto pr-1">
              {payload.items.map((item) => (
                <AttentionRow
                  ...
                  answers={answers}
                  onAnswerChange={(questionId, values) => setAnswers((current) => ({ ...current, [questionId]: values }))}
                  onAction={(action, actionPayload) => void runAction(item, action, actionPayload)}
                />
              ))}
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border p-10 text-center">
              <Check className="h-7 w-7 text-[hsl(var(--status-success-foreground))]" aria-hidden="true" />
              <p className="mt-3 text-sm font-semibold text-foreground">Nothing needs your attention</p>
              <p className="mt-1 max-w-sm text-xs leading-5 text-muted-foreground">{payload?.issues?.length ? "No verified pending items are currently available." : "OpenClaw has no current approvals, questions, suggestions, or actionable blockers."}</p>
            </div>
          )}
        </DialogContent>
      </Dialog>
  ...
  return (
    <article className="rounded-xl border border-border/80 bg-card/55 p-3">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        {secondaryActions.map((action) => action.id === "review" ? <Button key={action.id} asChild size="sm" variant="ghost" className="h-8 rounded-lg px-2.5 text-xs"><Link href="/tasks">{action.label}</Link></Button> : <Button key={action.id} size="sm" variant="ghost" className="h-8 rounded-lg px-2.5 text-xs" disabled={pending} onClick={() => onAction(action.id)}>{action.id === "deny" ? <X className="mr-1 h-3.5 w-3.5" aria-hidden="true" /> : null}{action.label}</Button>)}
      </div>
    </article>
  );
}
