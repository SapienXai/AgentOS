...
The 0.7.3 release expects Node.js 24 or newer and OpenClaw 2026.6.8 or newer. If compatibility is degraded, update OpenClaw, repair Gateway token/device access, restart the Gateway, and re-run `agentos doctor --deep`.
...
```bash
curl -fsSL https://raw.githubusercontent.com/SapienXai/AgentOS/main/install.sh | AGENTOS_VERSION=0.7.3 bash
```

Windows PowerShell:

```powershell
$env:AGENTOS_VERSION='0.7.3'; iwr https://raw.githubusercontent.com/SapienXai/AgentOS/main/install.ps1 | iex
```
...
git tag agentos-v0.7.3
git push origin agentos-v0.7.3
```
